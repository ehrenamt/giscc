...
AdjacencyList delaunyTriangulation2D(const std::vector<Point2D>& points) {
    ...
    for (auto& p : pointsCopy) {
        ...
        // if the circumcircle test fails, go to adjacency list, delete edge and add two new ones. Add the new
        // circumcircle tests?
...
bool pointInTriangle(Point2D& point, Triangle2D& triangle) {
    ...
    double alpha = ((triangle.b.y - triangle.c.y) * (point.x - triangle.c.x) +
                    (triangle.c.x - triangle.b.x) * (point.y - triangle.c.y)) /
                   denominator;
    double beta = ((triangle.c.y - triangle.a.y) * (point.x - triangle.c.x) +
...
bool pointInTriangle(Point2D& point, std::vector<Point2D>& triangle) {
    ...
    double denominator = (triangle[1].y - triangle[2].y) * (triangle[0].x - triangle[2].x) +
                         (triangle[2].x - triangle[1].x) * (triangle[0].y - triangle[2].y);

    double alpha = ((triangle[1].y - triangle[2].y) * (point.x - triangle[2].x) +
                    (triangle[2].x - triangle[1].x) * (point.y - triangle[2].y)) /
                   denominator;
    double beta = ((triangle[2].y - triangle[0].y) * (point.x - triangle[2].x) +
